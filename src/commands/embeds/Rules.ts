        ...
        await interaction.channel?.send({
            ...
            components: [
                new ActionRowBuilder<ButtonBuilder>().addComponents(
                    new ButtonBuilder().setLabel("J'ai lu le règlement").setEmoji(Emojis.Tick).setStyle(ButtonStyle.Success).setCustomId("rules")
                ),
            ],
        });

        return await interaction.reply({
            embeds: [new EmbedBuilder().setColor("Green").setDescription(`${Emojis.Tick} Règlement envoyé avec succès.`)],
            ephemeral: true,
        });
    }
}
